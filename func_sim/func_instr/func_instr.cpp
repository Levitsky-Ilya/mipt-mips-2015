/*
 * This is the module implementing the MIPS disassembler.
...
{
    mylog << "Entering Constuctor" << endl;
    ...
    {
		case FORMAT_R:
            if ( threeRegOperands( this->type))
            {
                ostr << asReg( this->bytes.asR.rd) << ", "
                     << asReg( this->bytes.asR.rs) << ", "
                     << asReg( this->bytes.asR.rt);
            }
            else if ( twoRegOperands( this->type))
            {
                ostr << asReg( this->bytes.asR.rd) << ", "
                     << asReg( this->bytes.asR.rs);
            }
            else if ( twoRegOperandsOneImm( this->type))
            {
                ostr << asReg( this->bytes.asR.rd) << ", "
                     << asReg( this->bytes.asR.rs) << ", "
                     << "0x" << hex << this->bytes.asR.rt;
            }
            else if ( oneRegOperand( this->type))
            {
                ostr << asReg( this->bytes.asR.rt);
            }
            else if ( oneRegOperandOneImm( this->type))
            {
                ostr << asReg( this->bytes.asR.rt) << ", "
            ...
            break;
        case FORMAT_I:
            if ( twoRegOperandsOneOffset( this->type))
            {
                ostr << asReg( this->bytes.asI.rt) << ", " 
            ...
            break;
        case FORMAT_J:
            ostr << hex << "0x" << this->bytes.asJ.offset;
            break;
        default:
            assert( 0);
    }

    full_cmd_name = ostr.str();
    mylog << "Exiting Contructor" << endl;
}
...
{
    mylog << "Entered R-parser" << endl;
    for ( unsigned i = 0; i < sizeof( isaTable) / sizeof( isaTable[0]); ++i)
    {
        mylog << "searching funct: " << this->bytes.asR.funct
            << ", curr funct: " << (int)isaTable[i].func
            << ", curr format: " << strFormat( isaTable[i].format)
            << endl;
...
string FuncInstr::asRegOff( unsigned char reg, uint16 offset) const
{
    ...
    ostr << "0x" << hex << offset << "(" << asReg( reg) << ")";
    return ostr.str();
}

string FuncInstr::strFormat( Format format) const
{
    switch( format)
    {
...
bool FuncInstr::threeRegOperands( Type type) const
{
...
bool FuncInstr::twoRegOperandsOneOffset( Type type) const
{
    return ( type==LB || type==LH || type==LW || type==LBU || type==LHU
            || type==SB || type==SH || type==SW);
}

bool FuncInstr::twoRegOperandsOneImm( Type type) const
{
...
bool FuncInstr::twoRegOperands( Type type) const
{
    return ( type==MULT || type==MULTU || type==DIV || type==DIVU);
}

bool FuncInstr::oneRegOperand( Type type) const
{
    return ( type==MFHI || type==MTHI || type==MFLO || type==MTLO
            || type == JR || type==JALR);
}

bool FuncInstr::oneRegOperandOneImm( Type type) const
{
